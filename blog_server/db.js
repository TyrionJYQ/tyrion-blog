...
obj.execBatchSql = (sqls, params) => {
    return new Promise((resolve, reject) =>{
        console.log('开始执行数据写入处理,参数:', JSON.stringify(sqls), JSON.stringify(params))
			...
			if(global.dblock.locked) {
		        ...
		        var process = function(err, rows){
		            ...
		            if(i >= sqls.length || hasFailed) {
		                if(!hasFailed) {
		                    console.log("处理数据库访问请求-批量执行插入、更新或删除语句成功",	JSON.stringify(sqls).substring(0, 500));
		                    db.run("COMMIT");
		                    global.dblock.locked = false
		                    resolve({})
		                }
		                else {
		                    console.log("处理数据库访问请求-批量执行插入、更新或删除语句失败，开始回滚", JSON.stringify(sqls).substring(0, 500));
		                    console.log(err);
		                    db.run("ROLLBACK");
		                    global.dblock.locked = false
		                ...
		                db.run(sqls[i], params[i], function(err, rows) {
		                    if(err){
		                        hasFailed = true;
		                        error = err;
		                        console.log("处理数据库访问请求-执行语句%s,参数:失败:", sqls[i], params[i].join(","), JSON.stringify(err));
		                    }
		                    else {
		                    	if(params[i] == undefined || params[i] == null) {
		                    		params[i] = [];
		                    	}
		                    	console.log("处理数据库访问请求-执行语句,参数:成功", sqls[i], params[i].join(","));
		                    }
		                    i++;
		                    process(err, rows);
		                });
